...
*   [x] `fprime-stm32` is registered as a reusable STM32 HAL library.
 ...
 *   [x] **Structure `fprime-stm32` Platform Library:** Create a reusable platform library (modeled after `fprime-vorago` or `fprime-arduino`) to house all low-level hardware abstraction logic.
